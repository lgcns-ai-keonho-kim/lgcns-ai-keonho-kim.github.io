...
## 1-1. 그래프 동작 이론 요약: 상태 중심 설계

그래프는 **상태(State)를 중심으로 노드들이 연결된 실행 흐름**입니다. 시작 노드가 초기 상태를 받고, 각 노드는 상태를 읽고 갱신해 다음 노드로 넘깁니다. 이때 노드 간 데이터 공유는 **항상 상태로만** 이루어지며, 노드 내부 변수는 전달되지 않습니다. 따라서 “다음 단계에 필요한 정보”를 상태 스키마로 먼저 정의하는 것이 가장 중요합니다. 이 구조를 이해하면 복잡한 분기나 재시도도 상태 값으로 단순화할 수 있습니다.

분기는 `add_conditional_edges`로 구현하지만, **실제 분기 기준은 상태 값**입니다. 즉, 이전 노드가 상태에 기록한 결과가 다음 경로를 결정합니다. 부분 업데이트를 사용할 경우 리듀서 병합 규칙이 필요하며, 없는 경우 전체 업데이트가 안전합니다. 또한 그래프 실행은 **요청 단위로 분리**되므로 상태는 실행 간에 공유되지 않습니다. 다만 노드 인스턴스가 내부 상태를 가진다면 실행 간 간섭이 생길 수 있으므로 무상태 설계가 권장됩니다.
